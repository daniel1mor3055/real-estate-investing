...
class Deal(BaseModel):
    ...
    @property
    def year_1_cash_flow(self) -> float:
        """Calculate Year 1 pre-tax cash flow."""
        return self.year_1_noi - self.financing.annual_debt_service
    
    @property
    def cap_rate(self) -> float:
        """Calculate capitalization rate."""
        if self.property.purchase_price > 0:
            return self.year_1_noi / self.property.purchase_price
        return 0
    
    @property
    def cash_on_cash_return(self) -> float:
        """Calculate cash-on-cash return."""
        if self.total_cash_needed > 0:
            return self.year_1_cash_flow / self.total_cash_needed
        return 0
